    ...
    def update(self):
        ...
        if self.time_remaining > 0:
            ...
            if pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT):
                ...
                if (self.hammer.is_hitting and not self.mole.is_hidden and
                    self.hammer.x > self.mole.molex and self.hammer.x < self.mole.molex + 10 and
                    self.hammer.y > self.mole.moley and self.hammer.y < self.mole.moley + 10):
                    
                    self.s.update(self.mole.c)
                    self.mole.is_hidden = True
        else:
            #タイムアップ
            self.Gameover=True
